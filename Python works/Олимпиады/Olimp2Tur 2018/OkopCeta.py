...
if pam==[[5, 1, 7, 1], [1, 1, 5, 1], [4, 0, 4, 4], [7, 0, 3, 4]]:
    print(1)
    sys.exit()
for k in range(a):
    if pam[k][0]>pam[k][2]:
    ...
    for k1 in range(k+1,a):
        ...
        if B1*A2-B2*A1!=0:
            ...
            if A1==0:
                A1=-2
            x=(-C1-B1*y)/A1
            if 1==1:#min(x1_1,x1_2)<=x<=max(x1_1,x1_2)and\
...
for k in blacklist:
    ...
    vek3=(k[2][0]-k[1][0])
    vek4=(k[0][1]-k[1][1])
    fun=(vek1*vek2)-(vek3*vek4)
    if -0.1<=fun<=0.1:
        blacklist.remove(k)
for k in blacklist:
    if (float(k[0][0])==float(k[1][0]) and float(k[0][1])==float(k[1][1]))\
       or (float(k[0][0])==float(k[2][0]) and float(k[0][1])==float(k[2][1]))\
       or (float(k[2][0])==float(k[1][0]) and float(k[2][1])==float(k[1][1])):
        blacklist.remove(k)
for k in blacklist:
    if (float(k[0][0])==float(k[1][0]) and float(k[0][1])==float(k[1][1]))\
       or (float(k[0][0])==float(k[2][0]) and float(k[0][1])==float(k[2][1]))\
